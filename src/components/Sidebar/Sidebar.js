...
const Sidebar = (props) => {
  ...
  const createLinks = (routes) => {
    ...
    // Hiding the Login Layout in the Auth
    return routes.filter((item)=> item.show !== false ).map((prop, key) => {
      if (prop.redirect) {
        return null;
      }
      if (prop.collapse) {
  ...
  if (logo !== undefined) {
    if (logo.outterLink !== undefined) {
      logoImg = (
        ...
        >
          <div className="logo-img" style={{backgroundColor: "#FFFFFF", borderRadius: "50%", height: "33px", width: "33px", paddingTop: "2px"}}>
            <img src={require("assets/img/favicon.png")} alt="react-logo" />
          </div>
        </a>
      );
      logoText = (
